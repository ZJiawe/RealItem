<template>
  <div class="login-container pull-height" @keyup.enter.native="handleLogin" v-if="showPrise">
...
export default {
  ...
  created() {
    ...
    var re = /access_token=\S*[&|\s]/g
    var hashstr = locationhash.match(re)
    var hash = ''
    if (hashstr.length > 0) {
      hash = hashstr[0]
    }
    if (hash.indexOf('access_token') >= 0) {
      ...
      console.log(hash)
      var tokenString = hash.substring(0, hash.indexOf('&'))
      console.log(tokenString)
      var token = tokenString.substr(tokenString.indexOf('=') + 1)
      console.log(token)
      setToken(token)
