package com.yihu.ehr.health.HealthCheckups.controller;
...
import com.yihu.ehr.health.HealthCheckups.service.HealthCheckupsService;
...
@Api(value = "HealthCheckups", description = "公卫系统-个人资料管理", tags = {"个人资料管理"})
public class HealthCheckupsEndPoint extends EnvelopRestEndPoint {
    @Autowired
    HealthCheckupsService HealthCheckupsService;

    @RequestMapping(value = ServiceApi.HealthCheckups.HealthCheckups, method = RequestMethod.GET)
    @ApiOperation(value = "获取体检表列表", notes = "根据查询条件获取体检表列表在前端表格展示")
    public Envelop searchUsers(
        ...
        try {
            ...
            List<HealthCheckups> userList = HealthCheckupsService.search(fields, filters, sorts, page, size);
            pagedResponse(request, response, HealthCheckupsService.getCount(filters), page, size);
    ...
    @RequestMapping(value = ServiceApi.HealthCheckups.HealthCheckups, method = RequestMethod.POST, consumes = MediaType.APPLICATION_JSON_UTF8_VALUE)
    ...
    public Envelop createUser(
        ...
        Envelop envelop = new Envelop();
        try {
            HealthCheckups user = toEntity(userJsonData, HealthCheckups.class);
            user.setId(getObjectId(BizObject.User));
            user.setCreateDate(new Date());
            user = HealthCheckupsService.saveUser(user);
            envelop.setSuccessFlg(true);
            envelop.setObj(user);
        } catch (IOException e) {
            envelop.setSuccessFlg(false);
    ...
    @RequestMapping(value = ServiceApi.HealthCheckups.HealthCheckups, method = RequestMethod.PUT, consumes = MediaType.APPLICATION_JSON_UTF8_VALUE)
    ...
    public Envelop updateUser(
        ...
        try {
            HealthCheckups user = toEntity(userJsonData, HealthCheckups.class);
            HealthCheckupsService.saveUser(user);
            envelop.setSuccessFlg(true);
            envelop.setObj(user);
        } catch (Exception e) {
            envelop.setSuccessFlg(false);
    ...
    @RequestMapping(value = ServiceApi.HealthCheckups.HealthCheckupsEntiry, method = RequestMethod.GET)
    ...
    public Envelop getUser(
        ...
        Envelop envelop = new Envelop();
        try {
            HealthCheckups user = HealthCheckupsService.getHealthCheckups(userId);
            envelop.setSuccessFlg(true);
            envelop.setObj(user);
        } catch (Exception e) {
            envelop.setSuccessFlg(false);
    ...
    @RequestMapping(value = ServiceApi.HealthCheckups.HealthCheckupsEntiry, method = RequestMethod.DELETE)
    ...
    public Envelop deleteUser(
        ...
        Envelop envelop = new Envelop();
        try {
            HealthCheckupsService.deleteUser(userId);
            envelop.setSuccessFlg(true);
        } catch (Exception e) {
            envelop.setSuccessFlg(false);
            envelop.setErrorMsg(e.getMessage());
    ...
    public void totalDelate (Perinfor entity){
        ...
        List<HealthCheckups> delateList = null;
        delateBuffer.append("peopleId=" + entity.getPeopleId() + ";");
        String delateFilters =  delateBuffer.toString();
        try {
            delateList = HealthCheckupsService.search("", delateFilters, "", 1, 500);                   //更改后统计实体
        } catch (ParseException e) {
            e.printStackTrace();
        }

        if(delateList.size()>0){
            for(HealthCheckups item:delateList){
                HealthCheckupsService.deleteUser(item.getId());
            }
